...
class Window(ttk.Window):
    def __init__(self):
        ...
        self.label_tuto = ttk.Label(
            self, text='ctrl + backspace p/ apagar tudo\nq para sair', foreground='gray')
        self.bt_config = ttk.Button(
            self, text='config', command=self.open_topbar)
        self.label_tuto.pack(side=LEFT)
        self.bt_config.pack(side=RIGHT)
